namespace OrderFlow.Ordering.Domain;
...
public sealed class Order : IHasDomainEvents
{
    ...
    public static Order Create(string reference, string? description, decimal value)
    {
        Validate(reference, description, value);
        var now = DateTimeOffset.UtcNow;
        var entity = new Order { Id = Guid.NewGuid(), Reference = reference.Trim(), Description = description?.Trim(), Value = value, Status = "Pending", CreatedAt = now, UpdatedAt = now };
        entity.Raise("ordering.order-created");
        return entity;
    }
    ...
    public void Update(string reference, string? description, decimal value, string status)
    {
        ...
        Raise("ordering.order-updated");
    }

    public void MarkDeleted() => Raise("ordering.order-deleted");
