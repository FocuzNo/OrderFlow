...
public sealed class CreateEndpoint(ISender sender) : Endpoint<CreateRequest, OrderDto>
{
    public override void Configure() { Post("/api/orders"); AllowAnonymous(); }
    public override async Task HandleAsync(CreateRequest r, CancellationToken ct) => await Send.ResponseAsync(await sender.Send(new CreateOrderCommand(r.Reference, r.Description, r.Value), ct), 201, ct);
}
public sealed class GetEndpoint(ISender sender) : Endpoint<IdRequest, OrderDto>
{
    public override void Configure() { Get("/api/orders/{id}"); AllowAnonymous(); }
    public override async Task HandleAsync(IdRequest r, CancellationToken ct) => await Send.OkAsync(await sender.Send(new GetOrderQuery(r.Id), ct), ct);
}
public sealed class ListEndpoint(ISender sender) : EndpointWithoutRequest<IReadOnlyList<OrderDto>>
{
    public override void Configure() { Get("/api/orders"); AllowAnonymous(); }
    public override async Task HandleAsync(CancellationToken ct) => await Send.OkAsync(await sender.Send(new ListOrdersQuery(), ct), ct);
}
public sealed class UpdateEndpoint(ISender sender) : Endpoint<UpdateRequest, OrderDto>
{
    public override void Configure() { Put("/api/orders/{id}"); AllowAnonymous(); }
    public override async Task HandleAsync(UpdateRequest r, CancellationToken ct) => await Send.OkAsync(await sender.Send(new UpdateOrderCommand(r.Id, r.Reference, r.Description, r.Value, r.Status), ct), ct);
}
public sealed class DeleteEndpoint(ISender sender) : Endpoint<IdRequest>
{
    public override void Configure() { Delete("/api/orders/{id}"); AllowAnonymous(); }
    public override async Task HandleAsync(IdRequest r, CancellationToken ct) { await sender.Send(new DeleteOrderCommand(r.Id), ct); await Send.NoContentAsync(ct); }
}
