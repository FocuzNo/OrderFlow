...
public sealed class StockItem : IHasDomainEvents
{
    ...
    public static StockItem Create(string reference, string? description, decimal value)
    {
        Validate(reference, description, value);
        var now = DateTimeOffset.UtcNow;
        var entity = new StockItem { Id = Guid.NewGuid(), Reference = reference.Trim(), Description = description?.Trim(), Value = value, Status = "Pending", CreatedAt = now, UpdatedAt = now };
        entity.Raise("inventory.stock-item-created");
        return entity;
    }
    ...
    public void Update(string reference, string? description, decimal value, string status)
    {
        ...
        Raise("inventory.stock-item-updated");
    }

    public void MarkDeleted() => Raise("inventory.stock-item-deleted");
