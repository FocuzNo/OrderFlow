...
{
    public CreateWarehouseValidator() { RuleFor(x => x.Name).NotEmpty().MaximumLength(120); RuleFor(x => x.Location).NotEmpty().MaximumLength(500); }
}
public sealed class CreateStockValidator : AbstractValidator<InventoryFeatures.CreateStock>
{
    public CreateStockValidator() { RuleFor(x => x.ProductId).NotEmpty(); RuleFor(x => x.WarehouseId).NotEmpty(); RuleFor(x => x.Sku).NotEmpty().MaximumLength(64); }
}
...
{
    public ReserveStockValidator() { RuleFor(x => x.OrderId).NotEmpty(); RuleFor(x => x.Quantity).GreaterThan(0); }
}
