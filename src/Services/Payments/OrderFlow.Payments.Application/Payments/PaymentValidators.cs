...
{
    public CreatePaymentValidator() { RuleFor(x => x.OrderId).NotEmpty(); RuleFor(x => x.Amount).GreaterThan(0); RuleFor(x => x.Method).NotEmpty(); }
}
public sealed class MarkPaymentSucceededValidator : AbstractValidator<PaymentFeatures.MarkSucceeded>
{
    public MarkPaymentSucceededValidator() => RuleFor(x => x.Reference).NotEmpty().MaximumLength(200);
}
...
{
    public RefundPaymentValidator() { RuleFor(x => x.Amount).GreaterThan(0); RuleFor(x => x.Reason).NotEmpty().MaximumLength(1000); }
}
