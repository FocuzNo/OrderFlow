...
public sealed class Payment : IHasDomainEvents
{
    ...
    public static Payment Create(string reference, string? description, decimal value)
    {
        Validate(reference, description, value);
        var now = DateTimeOffset.UtcNow;
        var entity = new Payment { Id = Guid.NewGuid(), Reference = reference.Trim(), Description = description?.Trim(), Value = value, Status = "Pending", CreatedAt = now, UpdatedAt = now };
        entity.Raise("payments.payment-created");
        return entity;
    }
    ...
    public void Update(string reference, string? description, decimal value, string status)
    {
        ...
        Raise("payments.payment-updated");
    }

    public void MarkDeleted() => Raise("payments.payment-deleted");
