...
{
    public sealed record Dto(Guid Id, string Name, string? Description)
    {
        public static Dto From(Category x) => new(x.Id, x.Name, x.Description);
    }

    public sealed record Create(string Name, string? Description) : ICommand<Dto>;

    public sealed class CreateHandler(ICategoryRepository r) : IRequestHandler<Create, Dto>
    {
