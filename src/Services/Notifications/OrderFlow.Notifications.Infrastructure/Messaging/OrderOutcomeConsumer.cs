...
) : BackgroundService
{
    ...
    )
    {
        ...
        for (var attempt = 1; attempt <= max; attempt++)
        {
            ...
            {
                logger.LogError(exception, "Notification event exhausted retries and moved to DLT");
    ...
    private async Task Handle(string content, CancellationToken cancellationToken)
    {
        ...
        if (
            envelope.EventType != nameof(OrderConfirmedIntegrationEvent)
            && envelope.EventType != nameof(OrderCancelledIntegrationEvent)
        )
            return;
        await using var scope = scopes.CreateAsyncScope();
        var databaseContext = scope.ServiceProvider.GetRequiredService<NotificationsDbContext>();
        if (
        ...
        string subject;
        string body;
        if (envelope.EventType == nameof(OrderConfirmedIntegrationEvent))
        {
            var integrationEvent =
                JsonSerializer.Deserialize<OrderConfirmedIntegrationEvent>(envelope.Payload)
                ?? throw new JsonException("Order event is invalid.");
            recipient = integrationEvent.CustomerEmail;
            subject = "Order confirmed";
            body = $"Order {integrationEvent.OrderId} has been confirmed.";
        }
        else
        {
            var integrationEvent =
                JsonSerializer.Deserialize<OrderCancelledIntegrationEvent>(envelope.Payload)
                ?? throw new JsonException("Order event is invalid.");
            recipient = integrationEvent.CustomerEmail;
            subject = "Order cancelled";
            body = $"Order {integrationEvent.OrderId} was cancelled: {integrationEvent.Reason}";
        }
        ...
        await sender.Send(
            new NotificationFeatures.SendNotificationCommand(notification.Id),
            cancellationToken
        );
